\revhist{9/13/89, pss; 4/11/91, pss; 6/29/91, pss; 7/30/92, pss; 3/23/94, pss;
         ...
         2/27/01, pss; 4/9/01, pss; 4/1/02, pss; 4/23/02, pss; 4/26/02, pss; 9/25/02, pss}
%
...
required to go through successive values toward zero as the shown ratio is watched.
The \Quote{limit} in \Eqnref{1} \parenhelp{7} means the value that the ratio approaches as the value of
\m{\Delta x} approaches zero. \help{6}}% /par
%
...
\help{2}
%
\Footnote{1}{The \textit{[S-2]} means that, if you need help, turn to the Special
Assistance Supplement at the end of this module and get advice from Sequence [S-2].}
%
\Eqn{2}{\dfrac{d(ax^p)}{dx}=pax^{p-1}.}
%
\Equationref{2} can be applied any time the derivative of any power of the
...
%
\Eqn{20}{\dfrac{dy(t)}{dt} = v_0 - g t.}}% /par
%}% /SubSect
%
...
application of \Eqnref{2} produces:
%
\Footnote{6}{This is the same as applying \Eqnref{2} once to \Eqnref{20}.}
%
\Eqn{}{\dfrac{d}{dt}\left(\dfrac{dy(t)}{dt}\right) = - g.}
%
The function \m{y(t)} we are using here is a polynomial of degree two, its first derivative is a
...
%
where \m{a}, \m{b}, and \m{c} are constants, can readily be shown to be: \help{4}
%
...
\Sect{4}{Indefinite and Definite Integrals}{\SectType{TextMultiPara}}{
%
...
The inverse of the derivative is what is needed, and the procedure is called
\Quote{\Index{integration}integration.}
The result of this procedure, the inverse of the derivative, is called
\Quote{\Index{integral}the integral} of the function.
For example, because \m{2x} is the derivative of \m{x^2} with respect to \m{x},
the integral of \m{2x} with respect to \m{x} is \m{x^2}.
However, \m{x^2 + 4} is also the integral of \m{2x}, as is \m{x^2 - 10}.
In fact, because the derivative eliminates any additive constant, the integral of \m{2x}
is \m{x^2 + C}, where \m{C} is an unknown constant.
Because of the presence of the indefinite constant, \m{x^2 + C} is called the
\Quote{indefinite integral} of \m{2x}.
This process of reversing differentiation, the indefinite integral, is customarily
written in the form:
%
\Eqn{}{\int  \dfrac{df(x)}{dx}\,dx = f(x) + C.}
%
That is, the indefinite integral of the derivative of some function \m{y(x)} is
just the function itself plus a constant.
In general, an indefinite integral is written as:
%
\Eqn{}{\int y(x)\,dx}
%
and it is up to the user to determine what function \m{y(x)} is a derivative of, using either
standard techniques or a symbolic computer program.
The function being integrated, \m{y(x)}, is called \Quote{the \Index{integrand}integrand of
the integral.}
Examples:
%
\begin{eqnarray*}
 ...
 (iii) & \int & \dfrac{dI(x)}{dx}dx = I(x) + C \qquad \text{for any } I(x). \\
\end{eqnarray*}
%
...
answers as illustrated in (iii).

\pcap[\Index{integration techniques}]{4}{b}{Other Integration Techniques}
Tables of integrals of functions of many kinds are available and are the
quickest technique for evaluating the integrals (anti-derivatives) of
whatever integrand may be at hand.
%
...
%
There are also computer programs that can find the anti-derivative for you.
%
\Footnote{8}{See \textit{Mathematica}, S.\,Wolfram, Addison-Wesley Publ.\,Co.\,(1991),
             \url{http://www.wolfram.com}, and \textit{Computing with MAPLE}, Francis Wright,
             Chapman \& Hall/CRC (2001) \url{http://www.maplesoft.com}. There are some
             400 books on MAPLE and at least as many on MATHEMATICA.}
%
\CaptionedFullFramedFigure{5}{The definite integral of \m{y(x)} from \m{x_0} to \m{x} is the area \m{A} that
                              is under the curve and bounded by \m{x_0} and \m{x}.}{m1gr05}

\pcap[\Index{integral, definite}\Index{definite integral}]{4}{c}{The Definite Integral as a Change in Quantity}
...
In all three cases we can say we have the rate of a quantity, say \m{dQ(t)/dt}, as a function of time and we wish to
find \m{\Delta\,Q\,\equiv Q(t_2) - Q(t_1)}, the change in total \m{Q} from time \m{t_1} to time \m{t_2}.
%
\Footnote{10}{A change in any quantity is commonly written using the upper case Greek letter \m{\Delta}.}
%
If the rate is constant then \m{\Delta Q} is just the rate times the time interval:
%
\Eqn{}{\Delta Q = \dfrac{dQ(t)}{dt} (t_2 - t_1)\,, \qquad \text{(constant rate)}}
%
...
\Quote{rate times time.}
If the rate is, say, \m{dQ/dt = a t^2 + b t^3}, then we can take the anti-derivative and get the amount as
a function of time: \m{Q(t) = (a/3)t^3 + (b/4)t^4 + C}.
The amount that \m{Q} changed from time \m{t_1} to time \m{t_2} is then the amount at time \m{t_2} minus the
amount at time \m{t_1}: \m{Q(t_2) - Q(t_1) = (a/3)(t_2^3-t_1^3) + (b/4)(t_2^4-t_1^4)}.
Note that the unknown constant \m{C} is gone and the answer is definite.
The result is called the \Quote{definite integral} and is written with the two times as the \Quote{lower limit}
and the \Quote{upper limit} of the integral:
%
\Eqn{}{\Delta Q = Q(t_2) - Q(t_1) \equiv \int_{t_1}^{t_2} \dfrac{dQ(t)}{dt}\,dt\,.}
%
We call the example \Quote{the integral from \m{t_1} to \m{t_2} of \m{(a t^2 + b t^3)}} and we write it as:
%
\Eqn{}{\Delta Q = \int_{t_1}^{t_2}\,(a t^2 + b t^3)\,dt\,.}
%
We write the \textit{result} of the integration this way:
%
\Eqn{}{\int_{t_1}^{t_2}\,(a t^2 + b t^3)\,dt = \left|(a/3)t^3 + (b/4)t^4\right|_{t_1}^{t_2}\,,}
%
...
In general,
%
\Eqn{}{\left|f(x)\right|^b_a \equiv f(b) - f(a)\,.}
%
The first vertical bar is sometimes omitted if that causes no ambiguity.
Note that vertical bars without limits indicate \Quote{absolute value,} a totally unrelated concept.
Then for our example:
%
\Eqn{}{\int_{t_1}^{t_2}\,(a t^2 + b t^3)\,dt = (a/3)(t_2^3-t_1^3) + (b/4)(t_2^4-t_1^4)\,.}
%
\tryit Show that:
%
\Eqn{}{\int_{1\unit{s}}^{2\unit{s}}\,(4\unit{m/s\up{4}} t^3 + 5\unit{m/s\up{5}} t^4)\,dt = 46\unit{m}\,.}

\pcap[\Index{integral, definite}\Index{definite integral}]{4}{d}{The Definite Integral as an Area}
The definite integral of a function between a lower and an upper limit equals the area under
the function's curve between those two limits (see \Figref{5}).
For proof, consider the graph of some function \m{y(x)} shown in \Figref{5}.
...
This amount \m{\Delta A} is greater than the area of the rectangle of area
\m{y(x)\,\Delta x} (the smaller dotted rectangle to the right of \m{A}) and less than the area of the
...
Thus for the graph of the \Quote{saw-tooth} function \m{y(x)}, shown in \Figref{6} in the interval
from \m{x = 0} to \m{x = 4a}, there is just as much area below the axis
(negative area) as there is above the axis (positive area), so they cancel
and the integral from 0 to \m{4a}  is zero.
In this interval of \m{x}, the saw-tooth function has three straight lines:
from \m{x = 0} to \m{x = a}, from \m{x = a} to \m{x = 3a}, and from \m{x = 3a} to \m{x = 4a}.
We integrate each of those lines and get the shaded area in \Figref{6}:
%
\TwoEqns{19}{\int_0^{4a} y(x)\,dx & = \int_{0}^{a} \dfrac{x}{a}\,dx
                                       + \int_a^{3a} \left(-\dfrac{x}{a} + 2\right)\,dx
                                       + \int_{3a}^{4a} \left(\dfrac{x}{a} -4\right)\,dx}
          {& = \dfrac{a}{2} + 0 - \dfrac{a}{2} = 0\,.}
%
\CaptionedFullFramedFigure{6}{Graph of a saw-tooth function illustrating that the
                              integral over the full cycle, 0 to \m{4a}, is zero.
                              The area above the axis, from \m{0} to \m{2a}, is equal to
                              the area below axis, from \m{2a}  to \m{4a}.}{m1gr07}

%
\tryit Show that each of the three linear functions in \Eqnref{19} does indeed properly describe its
portion of the saw-tooth function (an easy way is to plug in two different \m{x} values and show that
each produces a \m{y} value that is obviously on the corresponding line).
...
%
\Footnote{9}{See \Quote{Numerical Integration} (MISN-0-349).}
} %/Sect
%
